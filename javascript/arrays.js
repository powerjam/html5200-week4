...
*/
var scores = [12, 2, 300, 40, 23, 43, 53, 13, 25, 65, 9, 43, 23];
//console.log(scores);
...
function onImageClick(evt){
  ...
  //only want src attribute of image, so we need to get the "src" attribute of the img tag we click on
  var onlyPath = evt.target.getAttribute("src");
  //console.log(onlyPath);
  
  //indexOf array method searched for a match in the array; if it finds the data, it returns the index position in the array
  var imageIndex = imageListA.indexOf(onlyPath);
  //console.log(imageIndex);
  
  //get the element you clicked on OUT of the imageListA array, and save the clicked iamge path to a variable so we can use it later
...
function update_shopping_cart(){
  ...
  //create an output variable so we can build it op over the following lines
  var output = "";
  ...
  console.log(purchasedImages.length);
